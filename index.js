...
const addFunction = () => {
	...
	if (newTask) {
		...
		fs.writeFile(
			...
			newTask + '\n' + fileData,
			function (err) {
				// Keep variable for structure parity with tutorial-style code
				data = fileData.split('\n');
...
const doneFunction = () => {
	...
	if (doneIndex) {
		...
		// Convert it to string and slice only the date part, removing the time part
		...
		if (doneIndex > filterData.length || doneIndex <= 0) {
			...
			// Write the stored task in done.txt along with date string
			fs.writeFile(
...
switch (args[2]) {
	...
	default: {
		InfoFunction();
		// We will display help when no argument is passed or invalid argument is passed
	}
}
